...
export async function getServerSideProps({ req, query }) {
  ...
  // Fetch jobs by id
  const jobId = query.id; // Get job id from query parameters
  console.log("jobid",jobId)
  const jobRes = await fetch(`http://localhost:3000/api/auth/fetch_job_id?id=${jobId}`);
  ...
  const jobs = jobData.job ? [jobData.job].filter(job => !appliedJobIds.includes(job.id)) : [];
  return {
    props: {
      session,
      jobs
